...
class MaalerportalStatisticSensor(SensorEntity):
    ...
    async def _get_data(self, lastest_statistic: Optional[StatisticsRow]) -> None:
        ...
        last_sum = None
        if lastest_statistic is not None and lastest_statistic.get("sum") is not None:
            try:
                last_sum = float(lastest_statistic["sum"])
        ...
        for am in meter_readings:
            ...
            for reading in readings:
                ...
                if last_sum is not None and value < last_sum:
                    _LOGGER.warning(
                        "Ignoring decreasing reading for %s: %.3f -> %.3f",
                        self.entity_id,
                        last_sum,
                        value,
                    )
                    value = last_sum
                statistics.append(
                    StatisticData(start=hour_floor(reading.timestamp), sum=value)
                )
                last_sum = value
        ...
        if lastest_statistic is not None:
            ...
            if last_sum is not None:
                value = last_sum
            elif newest_reading is not None:
